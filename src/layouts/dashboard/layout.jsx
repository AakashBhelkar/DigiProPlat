          ...
          <HeaderBase
            ...
            slotProps={{
              toolbar: {
                sx: {
                  [`& [data-slot="logo"]`]: {
                    display: 'inline-flex', // Always show logo
                  },
                  [`& [data-area="right"]`]: {
                    gap: { xs: 0, sm: 0.75 },
                  },
                  ...(isNavHorizontal && {
                    ...
                    [`& [data-slot="divider"]`]: {
                      display: 'none', // Hide divider since we're not showing workspaces
                    },
                  }),
                },
              },
  ...
  return useMemo(() => {
    switch (settings.navColor) {
      ...
      case 'apparent':
        return {
          layout: {
            '--layout-nav-bg': palette.background.default, // Use light background instead of dark
            '--layout-nav-horizontal-bg': varAlpha(palette.background.defaultChannel, 0.96),
            '--layout-nav-border-color': varAlpha(palette.grey['500Channel'], 0.12),
            '--layout-nav-text-primary-color': palette.text.primary, // Use light text colors
            '--layout-nav-text-secondary-color': palette.text.secondary,
            '--layout-nav-text-disabled-color': palette.text.disabled,
            [stylesMode.dark]: {
              '--layout-nav-bg': palette.background.default, // Force light even in dark mode
              '--layout-nav-horizontal-bg': varAlpha(palette.background.defaultChannel, 0.96),
            },
          },
          section: {
